    ...
    <div className="page" style={{ paddingBottom: '5.5rem' }}>
      ...
      <div style={{ position: 'sticky', top: 0, zIndex: 10, background: 'var(--bg)', borderBottom: '1px solid var(--border)', padding: '0.85rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.6rem' }}>
          ...
          <button onClick={newTransaction} style={{ background: 'var(--accent)', border: 'none', color: 'var(--onAccent)', borderRadius: '7px', padding: '0.45rem 0.9rem', fontWeight: 700, fontSize: '0.82rem' }}>+ Log</button>
        </div>
      ...
      {showLog && (
        <div style={{ position: 'fixed', inset: 0, background: 'var(--scrim)', display: 'flex', alignItems: 'flex-end', zIndex: 50 }}
          ...
          <div style={{ background: 'var(--sheet)', borderTop: '2px solid var(--accent)', borderRadius: '16px 16px 0 0', padding: '1.25rem 1.25rem 2rem', width: '100%', maxWidth: '600px', margin: '0 auto', maxHeight: '85vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', gap: '0.4rem', marginBottom: '1rem' }}>
              {['expense','income','transfer'].map(t => (
                <button key={t} onClick={() => setForm(f => ({ ...f, type: t }))} style={{ flex: 1, background: form.type === t ? 'var(--accent)' : 'transparent', color: form.type === t ? 'var(--onAccent)' : 'var(--muted)', border: `1px solid ${form.type === t ? 'var(--accent)' : 'var(--border)'}`, borderRadius: '6px', padding: '0.4rem', fontSize: '0.75rem', fontWeight: form.type === t ? 700 : 400, textTransform: 'capitalize' }}>{t}</button>
              ))}
            </div>

      ...
      <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap', marginBottom: '1rem' }}>
        {people.map(p => (
          <button key={p.k} onClick={() => setFilter(p.k)}
            style={{ background: filter === p.k ? 'var(--accent)' : 'transparent', border: `1px solid ${filter === p.k ? 'var(--accent)' : 'var(--border)'}`, color: filter === p.k ? 'var(--onAccent)' : 'var(--muted)', borderRadius: '999px', padding: '0.3rem 0.75rem', fontSize: '0.75rem', fontWeight: filter === p.k ? 700 : 400 }}>
            {p.l}
          </button>
        ))}
      </div>
